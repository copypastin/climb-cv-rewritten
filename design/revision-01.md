...
**Coverage: every one of the 47 numbered findings (B1–B5, S6–S23, F-1..F-16, C-1..C-8) has a row below,
plus the 8 guardian notes, the 5 naming rulings, and the two Decision reviews.** Six carry a declined
component (F-6, F-16, half of S17, `join=` per ruling #4, and the two structural deferrals below); three
are deferred rather than refused (S9's `retain` kind, F-6's decimation, T2 itself). Nothing is partial
and nothing is unreached — rows with compound outcomes say so in the outcome cell rather than being
rounded to one word.

The pass was interrupted twice and resumed twice; **every finding now has a landing site.** The two
previously-flagged gaps are closed:

- `config-contract.md` — C-1 through C-8 and all four of `config.md` §13's asks.
- `plugin-api.md` — `join=` deleted (§4.3, §3.6, §6), S12 closed where it is *taught* (§4.2), §3.7–§3.11
  written, §5's template ids made placeholder, and **§7, the embedding API, written from scratch** (S19).

One item is deliberately **flagged as an open question rather than answered**: whether two `ClimbCV`
instances may run in one process (`plugin-api.md` §7.7). Nothing forbids it and nothing supports it;
`log_dir`, `state_dir` and the process-title convention would all collide. The proposed v1 answer is one
instance per process, enforced with a clear error — recorded as possibly deserving a Decision Log entry.
...
| **S12** `Meta.seq` claim is false on shared topics | **addressed** | Corrected where it is *stated* (`payloads.md` §2.3, `broker.md` §5.4) **and where it is taught** — `plugin-api.md` §4.2 rewritten with the `meta.source`-keyed idiom in code, both consequences spelled out (a decrease means restart *from that source*; cross-source `seq` comparison is meaningless), and a note on why the wording mattered more here than elsewhere: §6 hands this file to `docs-and-testing` as the whole of "how to write a plugin". |
...
| **S18** a blocking source cannot notice shutdown | **addressed** | `self.stopping` on the class surface (`plugin-api.md` §2) with semantics, the blocking-read idiom, and the "a read-only bool is not a concurrency primitive" narrowing of §3.6's over-broad blanket: `plugin-api.md` §3.10. Per-plugin `heartbeat_warn_s` companion: `isolation.md` §3.3, §5.4, `loader.md` §3.1. |
| **S19** the embedding API is an undesigned public surface | **addressed** | `plugin-api.md` §7, new, seven subsections. Answers all four of S19's questions: `required` defaults to **`False`** for host subscriptions with an always-fatal unknown-topic rule splitting out the typo case (§7.6); a host **may** subscribe to an absent topic, one INFO line (§7.6); **`ClimbCV.publish()` exists** and `<host>` enters exclusive contention as an ordinary candidate (§7.5); and `requires_topology` is a **mandatory keyword on `subscribe()`**, raising at the call site in the embedder's own traceback (§7.3) — which is what closes S19's sharpest point, that the highest-value check did not protect embedders at all. Also `<host>` in the resolution rules with its three differences and the reason for each (§7.2), callback threading and reentrancy (§7.4), and §7.7 listing what was **decided rather than derived**. The `spawn`-from-notebook verification S19 asked for is in `isolation.md` §2.4 and its cause is explained in §7.4 (host callbacks never cross a boundary). |
...
| template ids should be obviously placeholder | **addressed** | `plugin-api.md` §5 — templates renamed `my_logger`/`my_detector`, with the rename-prompt reasoning, the instruction that directory name / `id` / class name all carry the prefix, and three things both templates must contain (the `# No __init__` comment, `requires_topology` guidance, a commented `[config] keys`). Also `loader.md` §8. |

## Rulings

| Ruling | Outcome | Where |
|---|---|---|
| #1 keep `setup`/`teardown` | **accepted** | `plugin-api.md` §3.7 — kept, with the docstring caveat written out ("Called ONCE, in this plugin's own process, before any handler runs") plus the reason it is shouted: the vocabulary is borrowed from pytest/unittest where these run *per test*, so the carried intuition is "once per unit of work" and an author acting on it would reopen a camera every frame. |
...
| #4 cut `join=` | **addressed** | Deleted everywhere it was taught. `plugin-api.md` §4.3 is now the three-line `latest()` + `frame_seq` idiom the ruling accepted as sufficient, followed by all four reasons it was cut (shared-topic multiplicity, the `frame_seq == -1` replay-sentinel collision, breaking §3.3's fixed handler signature, and re-adding it later being a clean additive minor) plus the note that its multiplicity and `-1` semantics must be specified first if it ever returns. Also §3.6's omissions table and §6. Previously recorded in `isolation.md` §2.2 and `broker.md` §5.4. |
| #5 keep `finish()`, fix the contract | **addressed** | `isolation.md` §4.4.1 — escalation stated, plus the missing causal log message for the `finish()`-triggered shutdown, shaped like §5.3's. Legality in `setup()` settled (§4.4.2). Docstring half now in `plugin-api.md` §3.7, which shouts the escalation and tells the author to check `climbcv topics` for whether anything requires what they publish. |
| On #14 (full isolation) | **upheld** | Guardian's third argument (a forked *diagnostic* surface) is now the closing bullet of `loader.md` §2.1. **Both self-inflicted asymmetries are now closed**: the loader one by S20/F-3 (`loader.md` §2.1), the embedding one by S19 (`plugin-api.md` §7). |
...
| **F-4** GUI plugins must pump their own event loop | **addressed** | `plugin-api.md` §3.8 states it as a named rule in prose, with why the framework cannot do it centrally (pumping is toolkit-specific), why data stopping is not an edge case, the stash-plus-tick shape, the honest concession that this is `isolation.md` §3.3's blocking read leaking into the authoring interface (Decision #4 *bent*), and the corollary that the quit key must move to the timer too. Examples: `plugin-api.md` §2.1 (`PosePlot`, `ExoLive`). |
...
| **F-7** no per-plugin writable directory | **addressed** | `self.data_dir`, framework-created at `<state_dir>/<plugin_id>/`: `plugin-api.md` §2 and §3.9 (semantics, why F-3 promoted it from nice-to-have to required, and the explicit "not a sandbox" note), `broker.md` §6 (`PluginPlan.data_dir`), `isolation.md` §3.1 step 8, §8.1 (`state_dir` knob). Used in the `LidSensor` example. |
...
| **C-2** `config_dir` on `LoadedConfig` and the `Plugin` base | **addressed** | `config-contract.md` §1 (on `LoadedConfig`, `str \| None`) and §3.5 (the path-resolution asymmetry it fixes, and why the framework deliberately does *not* resolve plugin paths itself — that would need the typed schema Decision #8 excludes). Author-side in `plugin-api.md` §2 and §3.11 with the idiom; also `broker.md` §6. |
